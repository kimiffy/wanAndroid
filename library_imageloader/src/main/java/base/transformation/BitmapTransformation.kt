 ...
 * File Name:    BitmapTransformation.java
 * ClassName:    BitmapTransformation
 *
 * Description:
 *
 * @author 唐晓辉
 * @date 2021年09月16日 16:14
 */
abstract class BitmapTransformation : Transformation<Bitmap?> {
    override fun transform(context: Context,
                           resource: Resource<Bitmap?>, outWidth: Int,
                           outHeight: Int): Resource<Bitmap?> {
        require(Util.isValidDimensions(outWidth, outHeight)) { "Cannot apply transformation on width: $outWidth or height: $outHeight less than or equal to zero and not Target.SIZE_ORIGINAL" }
        val bitmapPool = Glide.get(context)
                .bitmapPool
